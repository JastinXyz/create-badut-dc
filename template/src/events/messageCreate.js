    ...
    async execute(message) {
        ...
        if (timestamps.has(message.author.id)) {
          ...
          if (now < expirationTime) {
            ...
            const rep = await message.replyWithoutMention({ content: message.client.config.messageContent.inCooldown.replace(/{username}/gm, message.author.username).replace(/{timestamp}/gm, `<t:${expiredTimestamp}:R>`) });
